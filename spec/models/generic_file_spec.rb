...
RSpec.describe GenericFile do
  ...
  context 'parent_collections_followers' do
    let(:user) { FactoryGirl.create(:user) }
    let(:gf) { make_generic_file(user) }

    describe 'with no parent collections' do
      it 'yields nothing' do
        expect { |b| gf.parent_collections_followers(&b) }.not_to yield_control
      end
    end

    describe 'with parent collections' do
      let(:user2) { FactoryGirl.create(:user) }
      let(:user3) { FactoryGirl.create(:user) }
      let(:user4) { FactoryGirl.create(:user) }
      let(:col2) { make_collection(user2, title: 'z') }
      let(:col3) { make_collection(user3) }
      let(:col4) { make_collection(user4, title: 'a') }
      ...
      context 'with followers' do
        ...
        it 'yields the collections and followers sorted by collection title' do
          expect {|b|
            gf.parent_collections_followers(&b)
          }.to yield_successive_args([col4, user], [col2, user3], [col2, user4])
        end
      end
    end
  end
