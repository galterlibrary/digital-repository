...
describe 'collection following event jobs' do
  ...
  context 'with generic file' do
    describe 'without parent collections' do
      it "logs content deposit events" do
        ...
        event = 'User <a href="/users/jill">jill</a> has deposited <a href="/files/test-123">Hamlet</a>'
        ...
        expect(@another_user.events.first[:action]).to eq(event)
        expect(@third_user.events.length).to eq(1)
        expect(@third_user.events.first[:action]).to eq(event)
        expect(@gf.events.length).to eq(1)
        expect(@gf.events.first[:action]).to eq(event)
      end
    end
    ...
    describe 'with parent collections and followers' do
      ...
      it "logs content deposit events for user and collection followers" do
        ...
        parent1_event = "#{event} for Collection: <a href='/collections/#{parent1.id}'>#{parent1.title}</a>"
        expect(follower1.events.first[:action]).to eq(parent1_event)

        expect(follower2.events.length).to eq(2)
        expect(follower2.events.second[:action]).to include(parent1_event)
        parent3_event = "#{event} for Collection: <a href='/collections/#{parent3.id}'>#{parent3.title}</a>"
        expect(follower2.events.first[:action]).to include(parent3_event)

        expect(follower3.events.length).to eq(1)
        expect(follower3.events.first[:action]).to eq(parent3_event)

        parent2_event = "#{event} for Collection: <a href='/collections/#{parent2.id}'>#{parent2.title}</a>"
        expect(follower4.events.length).to eq(1)
        expect(follower4.events.first[:action]).to eq(parent2_event)
      end
      ...
      it 'cares about permissions' do
        ...
        expect(follower2.events.length).to eq(2)
        expect(follower2.events.last[:action]).to include(parent1_event)
        parent3_event = "#{event} for Collection: <a href='/collections/#{parent3.id}'>#{parent3.title}</a>"
        expect(follower2.events.first[:action]).to include(parent3_event)
        ...
        parent2_event = "#{event} for Collection: <a href='/collections/#{parent2.id}'>#{parent2.title}</a>"
        expect(follower4.events.map!{ |e| e[:action] }).to include(parent2_event)
        parent4_event = "#{event} for Collection: <a href='/collections/#{parent4.id}'>#{parent4.title}</a>"
        expect(follower4.events.map!{ |e| e[:action] }).to include(parent4_event)
      end
    end
  end
  ...
  context 'with a collection' do
    ...
    let!(:parent3) { make_collection(create(:user)) }
    let!(:parent4) { make_collection(create(:user)) }
    let(:event) { 'User <a href="/users/jill">jill</a> has done something' }
    ...
    it 'processes collections and cares about permissions' do
      ...
      allow(job).to receive(:action).and_return(event)
      job.run
      ...
      expect(follower4.events.length).to eq(2)
      parent2_event = "#{event} for Collection: <a href='/collections/#{parent2.id}'>#{parent2.title}</a>"
      expect(follower4.events.last[:action]).to include(parent2_event)
      parent4_event = "#{event} for Collection: <a href='/collections/#{parent4.id}'>#{parent4.title}</a>"
      expect(follower4.events.first[:action]).to include(parent4_event)
    end
  end
end
