...
class UsersList
  ...
  def populate_user_info
    CSV.open(self.csv_file.path, "w") do |csv|
      # add headers
      csv << ['Formal Name', 'NetID', 'Email', 'Title', 'Address',
              'Sign in Count', 'Current Sign In', 'Last Sign In',
              'Total Files Deposited', 'Last Deposit Date']
      ...
      select_all_users.find_in_batches do |users_batch|
        users_batch.each do |user|
          ...
          csv << [user.formal_name, user.username, user.email, user.title,
                  user.address, user.sign_in_count, user.current_sign_in_at,
                  user.last_sign_in_at, count,
                  last_upload_date(solr_results, count)]
        end
      end
